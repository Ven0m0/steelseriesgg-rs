...
- **Memory Reuse**: The animation loop in `cmd_daemon` uses a `Vec<(String, Vec<Color>)>` declared outside the loop to reuse memory across frames, minimizing allocations by using `clear()` and `extend_from_slice()`. The `PerKeyEffectEngine` utilizes an in-place update strategy for `cached_key_colors` using `apply_effect_static`. `DaemonState::device_names` returns a `Vec<&str>` that borrows from internal info to avoid cloning.
- **Async I/O**: `ProfileManager` and `Config` provide asynchronous methods (`new_async`, `load_async`, etc.) using `tokio::fs` to prevent blocking the async runtime. Replace `Path::exists()` with `tokio::fs` operations that gracefully handle `ErrorKind::NotFound`. The `apply_per_key_effect_with_brightness` method in the `Keyboard` trait is asynchronous (via `#[async_trait]`).
- **Structs with Mutexes**: Structs containing `parking_lot::Mutex` (e.g., `HidOptimizer`, `PerformanceMonitor`) cannot automatically derive `Clone`; they must be shared via `Arc` or `OnceLock`.
